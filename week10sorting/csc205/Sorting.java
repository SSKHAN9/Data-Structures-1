...
public static <T extends Comparable<T>> void cutoff_qsort(T[] data) {
         ...
         cutoff_qsort(data,0,data.length-1,(data.length-(data.length/3)));
     }
...
void cutoff_qsort(T[] data, int min, int max, int cutoff) {
	// Project 7
	 if (min < max)
     {
          ...
          if(cutoff==countlimit)
          {
              sort(data);
              return;
          }
         cutoff_qsort(data, min, pivotIndex,cutoff);
         cutoff_qsort(data, pivotIndex + 1, max,cutoff);
     }
}
...
private static <T extends Comparable<T>> int Partition(T[] data, int min, int max)
{
    ...
    while (true)
    {
        do min++; while (data[min].compareTo(pivotValue) < 0) ;

        do max--; while (data[max].compareTo(pivotValue) > 0) ;
...
List<Integer> closestPair(Integer[] data) {
    ...
    Integer a=-1,b=-1,diff = 9999999;
      for(int i=0;i<data.length;i++)
     {
            for(int j=i+1;j<data.length;j++){
            	int temp = data[j] - data[i];
                if(temp < diff){
                        a=data[i];   
                        b=data[j];
                        diff = temp;
