...
public class LinkedList<T> implements Iterable<T> {
	...
	public T removeFirst() {
		...
		// Clear the references in the old head node
		oldHead.element = null;
		oldHead.next = null;
		
		size--;
		if (isEmpty()) {
			tail = null;   // tail should be null if list is empty
		}
	...
	public T removeLast() {
		...
		if (size == 1) {
			...
			//  next = tail
			ListNode<T> newTail = head;   // Start looking at the head
	...
	public Boolean contains(T value) {
		...
		while ((current != null) && !(found)) {
			found = current.element.equals(value);	// Check if the current node 
			                                        //   holds the element 
